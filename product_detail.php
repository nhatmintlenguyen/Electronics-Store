...
<nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-8">
    <a class="hover:text-primary" href="index.php"><?php echo t('home'); ?></a>
    <span class="material-symbols-outlined text-xs">chevron_right</span>
    <a class="hover:text-primary" href="products.php"><?php echo t('products'); ?></a>
    <span class="material-symbols-outlined text-xs">chevron_right</span>
    <a class="hover:text-primary" href="products.php?category=<?php echo $product['category_id']; ?>"><?php echo htmlspecialchars($product['category_name']); ?></a>
    <span class="material-symbols-outlined text-xs">chevron_right</span>
        ...
        <div class="sticky top-24 space-y-8">
            ...
            <div class="flex flex-col gap-4">
                <div class="flex gap-4">
                    ...
                    <button class="flex-1 bg-primary hover:bg-primary/90 text-white h-12 rounded-lg font-bold text-lg flex items-center justify-center gap-2 transition-all shadow-lg shadow-primary/20">
                        <span class="material-symbols-outlined">shopping_bag</span>
                        <?php echo t('add_to_cart'); ?>
                    </button>
                </div>
                
                <button class="w-full h-12 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg font-bold text-slate-700 dark:text-slate-300 flex items-center justify-center gap-2 transition-colors">
            ...
            <div class="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-800">
                ...
                <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                    <?php echo nl2br(htmlspecialchars($product['description'] ?? getCurrentLanguage() == 'vi' ? 'Sản phẩm chất lượng cao với công nghệ tiên tiến và thiết kế hiện đại.' : 'High quality product with advanced technology and modern design.')); ?>
                </p>
            </div>
...
<section class="mt-16 pt-12 border-t border-slate-200 dark:border-slate-800">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="product-card group relative bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-xl transition-all duration-300">
            <a href="product_detail.php?id=<?php echo $rel_product['id']; ?>" class="block">
                <div class="aspect-square bg-slate-50 dark:bg-slate-900 p-8 flex items-center justify-center relative overflow-hidden">
                    <?php if ($rel_product['image_url']): ?>
                        <img src="<?php echo htmlspecialchars($rel_product['image_url']); ?>" 
            ...
            <div class="p-5">
                <span class="text-[10px] font-bold text-primary tracking-widest uppercase mb-1 block">
                    <?php echo htmlspecialchars($rel_product['category_name']); ?>
                </span>
                <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-2 truncate">
                    <a href="product_detail.php?id=<?php echo $rel_product['id']; ?>">
                ...
                <div class="flex items-center gap-1 mb-3">
                    ...
                    <span class="text-xs font-bold text-slate-600 dark:text-slate-400">
                        <?php echo number_format($rel_product['rating'], 1); ?>
                    </span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-lg font-black text-slate-900 dark:text-white">
                        <?php echo formatPriceVND($rel_product['price']); ?>
                    </span>
                </div>
            </div>
        </div>
